// =========================================================
// silkpanda/momentum/app/components/meals/CreateRestaurantModal.tsx
...
interface CreateRestaurantModalProps {
    onClose: () => void;
    onRestaurantCreated: (restaurant: IRestaurant) => void;
}

const CreateRestaurantModal: React.FC<CreateRestaurantModalProps> = ({ onClose, onRestaurantCreated }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('/web-bff/meals/restaurants', {
                method: 'POST',
                headers: {
            ...
            onRestaurantCreated(data.data.restaurant);
            onClose();
        } catch (err: any) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <div className="relative w-full max-w-md p-6 bg-bg-surface rounded-xl shadow-xl border border-border-subtle" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    >
                        {isLoading ? <Loader className="w-5 h-5 animate-spin" /> : <><Check className="w-5 h-5 mr-2" /> Add Restaurant</>}
                    </button>
                </form>
            </div>
        </div>
