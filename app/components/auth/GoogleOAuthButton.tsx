// =========================================================
// app/components/auth/GoogleOAuthButton.tsx
// Google OAuth button that requests calendar permissions
// =========================================================
'use client';

import React from 'react';

interface GoogleOAuthButtonProps {
    text?: 'signin' | 'signup';
}

const GoogleOAuthButton: React.FC<GoogleOAuthButtonProps> = ({ text = 'signin' }) => {
    const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;

    const handleClick = () => {
        if (!clientId) {
            console.error('Google Client ID not configured');
            return;
        }

        const redirectUri = `${window.location.origin}/auth/google/callback`;

        // Request both authentication AND calendar scopes
        const scopes = [
            'openid',
            'email',
            'profile',
            'https://www.googleapis.com/auth/calendar.readonly',
            'https://www.googleapis.com/auth/calendar.events',
            'https://www.googleapis.com/auth/calendar',
        ];

        // Build OAuth URL
        const authUrl = new URL('https://accounts.google.com/o/oauth2/v2/auth');
        authUrl.searchParams.set('client_id', clientId);
        authUrl.searchParams.set('redirect_uri', redirectUri);
        authUrl.searchParams.set('response_type', 'code');
        authUrl.searchParams.set('scope', scopes.join(' '));
        authUrl.searchParams.set('access_type', 'offline'); // Get refresh token
        authUrl.searchParams.set('prompt', 'consent'); // Force consent screen
        authUrl.searchParams.set('state', text); // Pass signin/signup in state

        // Redirect to Google OAuth
        window.location.href = authUrl.toString();
    };

    if (!clientId) {
        return (
            <div className="p-4 bg-signal-alert/10 text-signal-alert rounded-lg border border-signal-alert/30 text-sm">
                Google Sign-In is not configured. Please add NEXT_PUBLIC_GOOGLE_CLIENT_ID to your environment variables.
            </div>
        );
    }

    return (
        <button
            onClick={handleClick}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border-2 border-border-subtle rounded-lg hover:bg-bg-canvas transition-colors shadow-sm"
        >
            <svg width="20" height="20" viewBox="0 0 24 24">
                <path
                    fill="#4285F4"
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                    fill="#34A853"
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                    fill="#FBBC05"
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                />
                <path
                    fill="#EA4335"
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                />
            </svg>
            <span className="text-text-primary font-medium">
                {text === 'signup' ? 'Sign up with Google' : 'Sign in with Google'}
            </span>
        </button>
    );
};

export default GoogleOAuthButton;
